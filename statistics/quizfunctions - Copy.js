...
function saveQuiz(){
	if (storage == 1){
		localStorage['SNOWSTATTEST']=document.getElementById('problemArea').innerHTML;
	}
}
//*********************************************************************************************************
...
function prepareToSubmit(){
	...
	alertText = '<b>'+quizId+'</b><br><b>Your Grade: '+GRADE+'%</b>';
	theProblemArea.innerHTML += alertText + '<br><p style="height: 30px;"></p>';
	
	theTestText='\n'+theProblemArea.innerHTML;

	document.getElementById('returnButton').style.display='inline';
	document.getElementById('calculatorButton').style.display='none';

	alert('You have completed this assignment.'+'\n'+'\n'+quizId+'\nYour Grade: '+GRADE+'%\n\nTo inform your instructor of your performance, you can send email. A copy of your assignment will automatically be included in the email.');
...
function displayProblems(){
	...
	for (i=1;i<N;i++){
		...
		tries[i]=0;
		grade[i]=-1;
		problemList+='<li id="problem'+i+'">'+question[i]+'<br>'+'<input type="text" id="response'+i+'">'
...
var bodyTextStart="";
var bodyTextLeft="	<div id='problemArea' style='position:fixed;top:0px;bottom:30px;left:0px;right:460px;overflow:auto;'>";
var bodyTextRight="	<div id='rightDiv' style='position: fixed; top: 0px; right: 0px; width:450px; background: white;overflow:auto;z-index:10;'></div>";
