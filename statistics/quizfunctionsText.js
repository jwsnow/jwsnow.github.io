...
function displayProblems(){
	...
	for (i=1;i<N;i++){
		...
		grade[i]=-1;
		if (type[i] != 'MC'){
			problemList+='NUM\t'+question[i]+'\t'+answer[i]+'\t'+epsilon[i]+'\r';
		}
		else{
			problemList+='MC\t'+question[i]+'\t';
			for (j in options[i]){
				problemList+=options[i][j]+'\t';
				if (j.toUpperCase()==answer[i].toUpperCase())
					problemList+='CORRECT\t';
				else
					problemList+='INCORRECT\t';
			}
			problemList+='\r';
		}
	}
	problemList=problemList.replace(/A\) /g,'');
	problemList=problemList.replace(/B\) /g,'');
	problemList=problemList.replace(/C\) /g,'');
	problemList=problemList.replace(/D\) /g,'');
	problemList=problemList.replace(/E\) /g,'');
	problemList=problemList.replace(/F\) /g,'');
	problemList=problemList.replace(/G\) /g,'');
	problemList=problemList.replace(/H\) /g,'');
	theDiv.innerHTML='<textarea style="width: 90%;height: 600px;white-space: nowrap;">'+problemList+'</textarea>';
	}
function displayProblemsWeb(){
	var i,j;
	var theDiv=document.getElementById('problemArea');
	var problemList='<ol>';
	for (i=1;i<N;i++){
