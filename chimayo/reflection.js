...
function fill8(x,y){
	...
	var dx=x-mx;
	var dy=y-my;
	fillrect(WIDTH+dx, my+dy,1,1);
	fillrect(WIDTH+dx, my-dy,1,1);
	fillrect(WIDTH-dx, my+dy,1,1);
	fillrect(WIDTH-dx, my-dy,1,1);
	fillrect(WIDTH+dy, my+dx,1,1);
	fillrect(WIDTH-dy, my+dx,1,1);
	fillrect(WIDTH+dy, my-dx,1,1);
	fillrect(WIDTH-dy, my-dx,1,1);
	
	fillrect(2*WIDTH-dx-1, my+dy,1,1);
	fillrect(2*WIDTH-dx-1, my-dy,1,1);
	fillrect(dx+1, my+dy,1,1);
	fillrect(dx+1, my-dy,1,1);
}
...
//if ((fr1.length>20) && (fr2.length>20))
	...
	for (a=WIDTH/2;a<WIDTH;a++){
		...
		for (b=a;b<WIDTH;b++){
			...
			fillcolor(cr,cg,cb);
			fillrect(2.5	*WIDTH-a,b,1,1);
			fillrect(2.5*WIDTH-a,WIDTH-b,1,1);
			fillrect(a-WIDTH/2,b,1,1);
			fillrect(a-WIDTH/2,WIDTH-b,1,1);
