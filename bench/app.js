import { GOOGLE_INK_RENDERER, GoogleInkStrokeModeler, modelGoogleInkStroke } from './google-ink-modeler.js?v=5.7.28';

const APP_VERSION = '5.7.28';
...
function bindStylusTouchInkFallback() {
  ...
  const activeGesture = () => state.inkGesture || state.eraserGesture || state.selectionGesture || state.regionCopyGesture;
...
function showDialog(kind) {
  ...
  if (kind === 'install') {
    ...
    els.dialogContent.innerHTML = `<h2>Milestone ${APP_VERSION}</h2>
      <p><strong>Development/diagnostic branch:</strong> official PDF Workbench remains 5.7.21 until this branch is promoted. The Presentation page-thumbnail navigator and Pages graph-paper background are now permanent Workbench features to retain. Milestone 5.7.28 retains the permanent Presentation thumbnail navigator and structured Pages graph-paper background, and keeps the temporary Pages migration button that removes only the exact legacy PowerPoint graph-paper JPEG from selected PDF pages. Transparent PDF areas reveal the procedural grid underneath; standard export writes the grid as vector PDF content behind imported PDF content. Editable full and selected-document backups now use ZIP DEFLATE compression, and the temporary diagnostics branch now records lightweight Pencil raw/coalesced-batch summaries so repeated/replayed Apple Pencil samples can be diagnosed from an ordinary class session. Pen/Pencil behavior itself, touch/pinch navigation, and annotation geometry are intentionally unchanged in this revision.</p>
