	...
	for (theChapter=0;theChapter<chapters.length;theChapter++){
		if (theChapter>0){
			if (chapters[theChapter].search(/^{([^}]*)}\s*\\label{([^}]*)}/) !=-1){
				...
				chapters[theChapter]=chapters[theChapter].replace(/^{([^}]*)}\s*\\label{([^}]*)}/, '<div class="chapter" id="$2"><div class="chapterHeading">Chapter '+theChapter+' $1</div>');
			}
			else
				chapters[theChapter]=chapters[theChapter].replace(/^{([^}]*)}/, '<div class="chapter"><div class="chapterHeading">Chapter '+theChapter+' $1</div>');
		...
		for (theSection=0;theSection<sect.length;theSection++){
			if (theSection>0){
				if (sect[theSection].search(/^{([^}]*)}\s*\\label{([^}]*)}/) !=-1){
					...
					sect[theSection]=sect[theSection].replace(/^{([^}]*)}\s*\\label{([^}]*)}/, '<div class="section" id="$2"><div class="sectionHeading">'+theChapter+'.'+theSection+' $1</div>');
				}
				else
					sect[theSection]=sect[theSection].replace(/^{([^}]*)}/, '<div class="section"><div class="sectionHeading">'+theChapter+'.'+theSection+' $1</div>');
