(function () {
  ...
  const STORAGE_KEY = "texesMath712Results.v1";
  ...
  function recordResult(result) {
    ...
    all.unshift({
      ...
      domainStats: (result.domainStats||[]).map(x=>({label:x.label,correct:x.correct,total:x.total,percent:x.percent}))
    });
    saveResults(all);
  }
  ...
  function renderHome() {
    ...
    main.innerHTML = `<section class="hero">
      <div class="hero-inner">
        <p class="eyebrow">TExES Mathematics 7–12 preparation</p>
        <h1>Diagnose every competency. Practice deeply. Understand every answer.</h1>
        <p>Original full-length diagnostics and focused quizzes for the complete Mathematics 7–12 (235) framework. Every question includes an explanation, and every full test reports performance by domain and by competency.</p>
        <div class="hero-note"><span aria-hidden="true">✓</span> All 21 competencies, including instruction and assessment</div>
      </div>
    </section>
  ...
  function submitQuiz(forced=false) {
    ...
    qz.questions.forEach((q,i)=>{
      const domain=q.domain||D.categories[q.categoryId]?.group||"Other";
      if(!domainMap.has(domain)) domainMap.set(domain,{label:domain,correct:0,total:0});
      const row=domainMap.get(domain); row.total++; if(qz.answers[i]===q.answer) row.correct++;
    });
    const domainStats=[...domainMap.values()].map(x=>({...x,percent:Math.round(100*x.correct/x.total)})).sort((a,b)=>a.label.localeCompare(b.label));
    const result={
      ...
      completedAt:new Date().toISOString(),durationSeconds:qz.elapsedSeconds,categoryStats,domainStats,
  ...
  function renderResults() {
    ...
    <section class="content-wrap">
      ...
      <section class="panel">
        <h2>Performance by official domain</h2>
        <table class="domain-table"><thead><tr><th>Domain</th><th>Performance</th><th>Score</th></tr></thead><tbody>
          ${(r.domainStats||[]).map(x=>`<tr><td>${escapeText(x.label)}</td><td><div class="bar" aria-label="${x.percent}%"><span style="width:${x.percent}%"></span></div></td><td>${x.correct}/${x.total} (${x.percent}%)</td></tr>`).join("")}
        </tbody></table>
      </section>
      <section class="panel">
        <h2>Performance by competency</h2>
  ...
  function renderAbout() {
    main.innerHTML=`<section class="content-wrap">${breadcrumbs("About")}
      ...
      <section class="panel"><h2>Scoring limitations</h2><p>The practice percentages are raw scores. TExES reports a scaled score, and this independent site cannot reproduce Pearson's scoring model or predict a passing result. Use the reports to identify priorities and measure growth.</p></section>
